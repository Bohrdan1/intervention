...
const DOSSIER_STATUT_CONFIG: Record<string, { label: string; badge: string }> = {
...
export function FacturationSection({ dossierId, facturation, statut }: Props) {
  const [editing, setEditing] = useState(false);
  const [isPending, startTransition] = useTransition();
  const router = useRouter();
  const [isOffert, setIsOffert] = useState(facturation.offert ?? false);

  const statutCfg =
    DOSSIER_STATUT_CONFIG[statut] ??
    ({ label: statut, badge: "bg-gray-100 text-gray-600" } as const);

  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    const offert = fd.get("offert") === "on";

    const data = {
      offert,
      facture_numero: offert ? null : (fd.get("facture_numero") as string) || null,
      facture_date:   offert ? null : (fd.get("facture_date") as string) || null,
      facture_montant_ttc: offert ? null : (() => {
        const v = (fd.get("facture_montant_ttc") as string) || "";
        const n = parseFloat(v.replace(/\s/g, ""));
        return isNaN(n) ? null : n;
      })(),
      reglement_date: offert ? null : (fd.get("reglement_date") as string) || null,
      reglement_mode: offert ? null : (fd.get("reglement_mode") as string) || null,
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex items-center gap-3 py-2">
            <input
              ...
              name="offert"
              checked={isOffert}
              onChange={(e) => setIsOffert(e.target.checked)}
            ...
            >
              Prestation offerte
              <span className="text-xs text-muted ml-1">(sans facturation)</span>
            </label>
          </div>
    ...
    <div className="rounded-xl border border-border bg-white p-4">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between">
          ...
          {facturation.offert ? (
            <span className="rounded-full px-2.5 py-0.5 text-xs font-semibold bg-purple-100 text-purple-700">
              🎁 Offert
            </span>
          ) : (
            <span
              className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${statutCfg.badge}`}
            >
              {statutCfg.label}
            </span>
          )}
