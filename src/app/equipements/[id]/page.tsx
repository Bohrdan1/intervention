...
export default async function HistoriqueEquipementPage({
  params,
}: {
  ...
  // Charger l'équipement avec son site et client
  ...
  const site = Array.isArray(equipement.site) ? equipement.site[0] : equipement.site;
  ...
  // 2. Interventions directes sur cet équipement
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-muted">
        ...
        <span>›</span>
        {client && (
          <Link href={`/clients/${client.id}`} className="hover:text-foreground font-medium">
            {client.nom}
          </Link>
        )}
        {site && <><span>›</span><span>{site.nom}</span></>}
        <span>›</span>
        <span className="text-foreground font-medium">{equipement.repere}</span>
      </nav>
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold flex items-center gap-2">
              🚪 {equipement.repere}
              {equipement.avec_batterie && (
                <span className="text-sm font-normal text-amber-600">🔋 Batterie</span>
              )}
            </h1>
            <p className="text-sm text-muted mt-0.5">
              {equipement.type_porte}
              {equipement.modele ? ` · ${equipement.modele}` : ""}
              {equipement.marque ? ` · ${equipement.marque}` : ""}
            </p>
            {equipement.numero_serie && (
              <p className="text-xs text-muted">N° série : {equipement.numero_serie}</p>
            )}
            {equipement.annee_installation && (
              <p className="text-xs text-muted">Installé en {equipement.annee_installation}</p>
            )}
            ...
            {equipement.notes_techniques && (
              <p className="mt-1 rounded border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs text-blue-800">
                🔧 {equipement.notes_techniques}
              </p>
            )}
          </div>
      ...
      {timeline.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-border bg-white p-10 text-center">
          ...
          <p className="text-sm text-muted">Aucun rapport pour cet équipement.</p>
        </div>
      ) : (
                ...
                <div key={`${evt.type}-${i}`} className="flex gap-4">
                  ...
                  <div className={`flex-1 mb-1 rounded-xl border bg-white p-4 shadow-sm ${
                    ...
                    {evt.type === "maintenance" && (() => {
                      ...
                      const anomalies = pts.filter((p) => p.etat !== "ok" && p.etat !== "na");
                      return (
